...
_MM_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
# 默认加入本仓库 mm-neurons 下的 open_source_model；若你单独克隆了 LLaVA 等，请改为绝对路径并取消注释
for _rel in (
    ("open_source_model", "LLaVA-main"),
    ("open_source_model", "InstructBlip"),
    ("open_source_model", "mPLUG-Owl-main", "mPLUG-Owl2"),
):
    _p = os.path.join(_MM_ROOT, *_rel)
    if os.path.isdir(_p):
        sys.path.append(_p)
# USER: 若默认路径下无对应仓库，请手动添加，例如：
# sys.path.append(r"")
