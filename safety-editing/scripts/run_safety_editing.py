...
# safety-editing 根目录（含 easyeditor 包），勿依赖服务器上的绝对路径
...
# USER: 按需设置 GPU；也可在启动前 export CUDA_VISIBLE_DEVICES
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "0")
# USER: 可选，Hugging Face / datasets 等缓存根目录（不设置则使用系统默认）
...
from easyeditor import DINMHyperParams, MENDTrainingHparams, FINEHyperParams, FTHyperParams, LoRAHyperParams
...
def test_detoxification(editing_method, edit_data_all, editor, hparams, safety_classifier_model, safety_classifier_tokenizer, detoxify_metric, output_dir, train_ds, args):
    overall_performance = []
    edit_data_all = edit_data_all[:]
    for i, data in tqdm(enumerate(edit_data_all), total=len(edit_data_all)):
        ...
        for item in metrics:
            ...
            write_json(f'{output_dir}', item_evaluate, case_id = [i], data_all = len(edit_data_all))
...
if __name__ == '__main__':
    ...
    for name in ("safety_classifier_dir", "data_dir", "metrics_save_dir"):
        if not getattr(args, name).strip():
            raise SystemExit(
                f"请在命令行设置 --{name.replace('_', '-')}=<路径>，或在脚本中填写占位默认值（见 argparse 注释）。"
            )
    ...
    output_dir = f'{args.metrics_save_dir}/{args.editing_method}_{args.edited_model}_time_{datetime.now().strftime("%Y-%m-%d_%H:%M:%S")}.json'
    ...
    if args.editing_method == 'IKE':
        train_ds = SafetyDataset(f'{args.data_dir}/SafeEdit_train.json')
    ...
    if args.editing_method != "MEND":
        overall_performance = test_detoxification(args.editing_method, edit_data_all, editor, hparams, safety_classifier_model, safety_classifier_tokenizer, detoxify_metric, output_dir, train_ds, args)
    ...
    with open(output_dir, 'w+') as f:    
        json.dump(new_output, f, ensure_ascii=False, indent=4)
