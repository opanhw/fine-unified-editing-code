...
def compute_zs(
    ...
    if "neo" in model.config._name_or_path or "llama" in model.config.model_type or "qwen" in model.config.model_type:
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        log_probs = torch.log_softmax(ln_f(full_repr) @ lm_w + lm_b.to(lm_w.device), dim=2)
        # if it == 0:
        #     torch.save(input_tok["input_ids"], "")  # USER: debug path
        #     torch.save(full_repr, "")  # USER: debug path

        # print(log_probs)
        # print(log_probs.shape)
        # print(rewriting_targets.shape)
        # exit(0)
        ...
        loss = nll_loss + kl_loss + weight_decay.to(kl_loss.device)
