...
def apply_AlphaEdit_to_model(
  ...
  #-> Tuple[AutoModelForCausalLM, Dict[str, Any]]:
    ...
    if not os.path.exists(hparams.P_loc):
        os.makedirs(hparams.P_loc, exist_ok=True)
    if not os.path.exists(hparams.P_loc + "/null_space_project.pt"):
        ...
        torch.save(P, hparams.P_loc + "/null_space_project.pt")
        P_loaded = True
    elif P_loaded == False:
        P = torch.load(hparams.P_loc + "/null_space_project.pt")
        P_loaded = True
...
def execute_AlphaEdit(
    ...
    for i, request in enumerate(requests):
        ...
        request["subject"] = tok.batch_decode([tok.encode(request["prompt"], add_special_tokens=False)[-1]])[0]

        request['prompt'] = request['prompt'].replace("{", "<").replace("}", ">")

        request['prompt'] = tok.batch_decode([tok.encode(request["prompt"], add_special_tokens=False)[:-1]])[0] + "{}"
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        upd_matrix = torch.linalg.solve(
                ...
                P[i,:,:].to(f"cuda:{hparams.device}") @ layer_ks.to(P[i,:,:].dtype).to(f"cuda:{hparams.device}") @ resid.T.to(f"cuda:{hparams.device}")
        )
