...
def test_safety_gen(
    ...
    if max_tokens < 1624:
        ...
        # print(test_prompt)
        # print(len(test_prompt))
        # exit(0)
        input = tokenizer(test_prompt, return_tensors="pt", padding=True, truncation=True).to(model.device)
        # print(model.get_input_embeddings().weight.shape)
        # print(model.get_output_embeddings().weight.shape)
        # print(input["input_ids"])
        # print(input["input_ids"].shape)
        # print(input["input_ids"].min(), input["input_ids"].max())
        ...
        # exit(0)
        with torch.no_grad():
            # model.generate(**tokenizer(test_prompt[0], return_tensors="pt", padding=True, truncation=True).to(f"cuda:{cuda}"), max_new_tokens=max_output_tokens)
            # print(0)
            # model.generate(**tokenizer(test_prompt[1], return_tensors="pt", padding=True, truncation=True).to(f"cuda:{cuda}"), max_new_tokens=max_output_tokens)
            # print(1)
            # model.generate(**tokenizer(test_prompt[2], return_tensors="pt", padding=True, truncation=True).to(f"cuda:{cuda}"), max_new_tokens=max_output_tokens)
            # print(2)
            # model.generate(**tokenizer(test_prompt[3], return_tensors="pt", padding=True, truncation=True).to(f"cuda:{cuda}"), max_new_tokens=max_output_tokens)
            # print(3)
            # model.generate(**tokenizer(test_prompt[4], return_tensors="pt", padding=True, truncation=True).to(f"cuda:{cuda}"), max_new_tokens=max_output_tokens)
            # print(4)
            outputs = model.generate(**input, max_new_tokens=max_output_tokens)
            texts = [tokenizer.decode(output, skip_special_tokens=True) for output in outputs]
